  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiFetch<{ logs: ApiKeyLog[]; total: number }>(
        `/api/repo/${org}/${repo}/logs?${params.toString()}`
      );
      setLogs(data.logs);
      setTotal(data.total);
    } catch (error) {
      ...
      <div className="h-full overflow-y-auto">
        <div className="mx-auto max-w-6xl px-4 sm:px-8 py-6 sm:py-10">
          <div className="animate-fade-in space-y-4 sm:space-y-6">
            ...
            {/* Tabs */}
            <RepoTabs org={org} repo={repo} activeTab="logs" />
            ...
            {loading && logs.length === 0 ? (
              ...
              <div className="flex h-64 flex-col items-center justify-center gap-2 rounded-xl border border-border/30 bg-dracula-current">
                ...
                <p className="text-[12px] text-muted-foreground/70 text-center px-4">
                  Logs will appear here when API keys are used for this repository
                </p>
              </div>
            ) : (
              <>
