...
import type { ConflictResolution, Memory } from "unforgit-shared";
...
describe("LocalStore remote conflict resolution", () => {
  let tmpDir: string;
  let store: LocalStore;
  let local: Memory;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "unforgit-local-sync-"));
